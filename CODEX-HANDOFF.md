...
2. **Test-harness failures to investigate separately** (see TESTING.md):
   ...
   - resolved: repeated same-hash source-link recovery. Evidence's immediate scrolling (`homepage.css`) prevents it, and `sofiya.spec.js` now fails if Evidence jumps animate; keep that rule when merging theme CSS.
