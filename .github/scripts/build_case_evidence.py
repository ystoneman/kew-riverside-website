...
<p class="section-lead">Kew Riverside had a revenue reserve of <strong>{money(exact)}</strong> on 31 March 2026, after a <strong>{money(fall)}</strong> reduction that year. The council projects exhausted reserves by March {exhaustion_year} and a deficit of more than {bound} by {e(statements[1]['targetFinancialYear'])}. These are different points in time. The annual budget connecting them—income, spending, commitments, support and pupil assumptions—has not been supplied in the reviewed public records. The reserve alone does not establish financial sustainability.</p>
<span class="data-date">Actual to 31 March 2026 · projection to 2028/29 · checked {e(data['checkedOn']['finance'])}</span>
{sequence}
<p class="chart-footnote">Sources: {source_link(data, 'school-balances-mar-2026', 'council school balances, Kew row')} and {source_link(data, 'consultation-leaflet', 'Kew consultation leaflet')}. The council reports revenue and capital balances separately; the figure above is the <strong>revenue</strong> reserve. Its three-year deficit claim is a projection, not a later actual or a measured amount needed to keep the school open.</p>
<details class="data-disclosure" id="budget-actuals"><summary>View five years of income, spending and reserves</summary><p>Annual result means income less expenditure in that financial year. Revenue reserve is the balance carried at year end. These DfE history figures are reported at the source's displayed precision; they are not an annual forecast.</p>{history}<p>Sources: {source_link(data, 'kew-finance-income-history', 'DfE income history')}, {source_link(data, 'kew-finance-expenditure-history', 'expenditure history')} and {source_link(data, 'kew-finance-balance-history', 'balance history')}. For 2025/26 the DfE whole-pound in-year result is −£75,103; the council's precise reserve movement is −£75,103.70, rounded to −£75,104. Their displayed whole-pound figures therefore differ by £1. The bridge to the later forecast remains unavailable.</p><p><a href="case-evidence.csv" download="case-evidence.csv">Download these observations and their sources (CSV) ↓</a> · <a href="case-evidence-data.json" download="case-evidence-data.json">Structured observations (JSON)</a></p></details>
<div class="case-subsection" id="closure-costs"><h3>What would closure actually save?</h3><p>The DfE records a <strong>{money(school_pfi['amountPence'], school_pfi['sourcePrecision'])}</strong> PFI charge in Kew Riverside's <strong>{e(school_pfi['financialYear'])}</strong> expenditure. That school charge is not the council's entire contract liability or a saving that would automatically follow closure. Which costs would cease, continue or transfer—and what new transition, receiving-school, support or transport costs would arise? The net comparison and Kew-specific contract terms were not in the reviewed public records. {source_link(data, school_pfi['sourceId'], 'DfE expenditure history')} · <a href="options.html#option-5">See the cost audit question</a>.</p></div>
<aside class="reading-note"><p><strong>The missing financial test:</strong> ask for the approved 2026/27 budget, monitoring and annual forecast that reconcile the dated reserve with the council's projected deficit, plus a comparable net cost for each realistic alternative. <a href="proposal.html#questions">Use this in a consultation response</a>.</p></aside></section>'''
...
<p class="section-lead">For <strong>{e(check['targetAcademicYear'])} in the Kew planning area</strong>, a published forecast was {total['forecastPupils']} Reception–Year 6 pupils; the January 2026 census counted {total['actualPupils']}. The total was close, while the Reception forecast of {reception['forecastPupils']} was {reception['actualPupils'] - reception['forecastPupils']} below the actual {reception['actualPupils']}. This checks one forecast vintage and one period across three schools, not a Kew Riverside-only forecast or long-term accuracy test.</p>
<span class="data-date">2024/25 SCAP forecast submission · January 2026 actual · checked {e(data['checkedOn']['forecasts'])}</span>
{comparison}
<p class="chart-footnote">Sources: {source_link(data, check['forecastSourceId'], 'DfE planning-area forecasts')} and {source_link(data, check['actualSourceId'], 'DfE January 2026 school census')}. Planning-area code {e(check['planningAreaCode'])}; Kew Riverside, Darell and The King's; nursery excluded. “Actual minus forecast” is a count, not a percentage error.</p>
<div class="case-subsection"><h3>Why did the later forecast change?</h3><p>For the same Kew planning area and target year <strong>{e(revision['targetAcademicYear'])}</strong>, the {e(revision['earlierPublication'])} projected <strong>{revision['earlierPupils']}</strong> pupils; the {e(revision['laterPublication'])} projected <strong>{revision['laterPupils']}</strong>, an increase of <strong>{changed}</strong>. This forecast revision is not itself a measured forecasting error. Which population, housing, admissions or method assumptions changed, and which version underpins the proposal and school budget? {source_link(data, revision['earlierSourceId'], 'June report')} · {source_link(data, revision['laterSourceId'], 'consultation leaflet')}.</p></div>
...
<aside class="reading-note"><p><strong>A useful question for the council:</strong> provide its checks by forecast vintage, horizon and year group, and explain the assumptions behind the revised Kew-area series. <a href="proposal.html#questions">Read the consultation questions</a>.</p></aside></section>'''
